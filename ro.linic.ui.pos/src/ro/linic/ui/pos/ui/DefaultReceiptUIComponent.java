...
public class DefaultReceiptUIComponent implements ReceiptUIComponent {
	...
	private static final Column nameColumn = new Column(1, Product.NAME_FIELD, Messages.Name, 350);
	private static final Column uomColumn = new Column(2, Product.UOM_FIELD, Messages.UOM, 70);
	private static final Column priceColumn = new Column(3, Product.PRICE_FIELD, Messages.Price, 90);
...
//	private TreeOperationsNatTable receiptTable;
...
//		receiptTable = new TreeOperationsNatTable(TreeOperationsNatTable.SourceLoc.VANZARI);
//		receiptTable.postConstruct(verticalSash);
//		receiptTable.getTable().setBackground(Display.getDefault().getSystemColor(SWT.COLOR_WHITE));
//		GridDataFactory.fillDefaults().grab(true, true).applyTo(receiptTable.getTable());
