...
import ro.linic.ui.p2.internal.ui.RepositoryOperationBeginningEvent;
import ro.linic.ui.p2.internal.ui.RepositoryOperationEndingEvent;
import ro.linic.ui.p2.internal.ui.dialogs.InstallWizard;
import ro.linic.ui.p2.internal.ui.dialogs.PreselectedIUInstallWizard;
import ro.linic.ui.p2.internal.ui.dialogs.ProvisioningWizardDialog;
import ro.linic.ui.p2.internal.ui.dialogs.UninstallWizard;
import ro.linic.ui.p2.internal.ui.dialogs.UpdateSingleIUWizard;
import ro.linic.ui.p2.internal.ui.dialogs.UpdateWizard;
...
public class ProvisioningUI {
	 ...
	 */
	public static ProvisioningUI getDefaultUI(final IEclipseContext ctx) {
		return ctx.get(ProvisioningUI.class);
	}
	 ...
	 */
	public ProvisioningUI(final IEclipseContext ctx, final ProvisioningSession session, final String profileId, final Policy policy) {
		this.ctx = ctx;
		this.policy = policy;
		...
		this.runner = new ProvisioningOperationRunner(ctx, this);
	...
	public LicenseManager getLicenseManager() {
		return ServiceHelper.getService(FrameworkUtil.getBundle(this.getClass()).getBundleContext(), LicenseManager.class);
	}
	 ...
	 */
	public int openInstallWizard(final Collection<IInstallableUnit> initialSelections, final InstallOperation operation,
	 ...
	 */
	public int openInstallWizard(final Collection<IInstallableUnit> initialSelections, final InstallOperation operation,
			final RemediationOperation remediationOperation, final LoadMetadataRepositoryJob job) {
		if (operation == null) {
			final InstallWizard wizard = new InstallWizard(ctx, this, operation, initialSelections, job);
			final WizardDialog dialog = new ProvisioningWizardDialog(ctx, ProvUI.getDefaultParentShell(), wizard);
			dialog.create();
//			PlatformUI.getWorkbench().getHelpSystem().setHelp(dialog.getShell(), IProvHelpContextIds.INSTALL_WIZARD);
			return dialog.open();
		}
		final PreselectedIUInstallWizard wizard = new PreselectedIUInstallWizard(ctx, this, operation, initialSelections, job);
...
//		PlatformUI.getWorkbench().getHelpSystem().setHelp(dialog.getShell(), IProvHelpContextIds.INSTALL_WIZARD);
	 ...
	 * @return the wizard return code
	 */
	public int openUpdateWizard(final boolean skipSelectionsPage, final UpdateOperation operation, final LoadMetadataRepositoryJob job) {
	 ...
	 * @since 2.3
	 */
	public int openUpdateWizard(final boolean skipSelectionsPage, final UpdateOperation operation,
			final RemediationOperation remediationOperation, final LoadMetadataRepositoryJob job) {
		if (getPolicy().getUpdateWizardStyle() == Policy.UPDATE_STYLE_SINGLE_IUS
				&& UpdateSingleIUWizard.validFor(operation)) {
			final UpdateSingleIUWizard wizard = new UpdateSingleIUWizard(ctx, this, operation);
			final WizardDialog dialog = new WizardDialog(ProvUI.getDefaultParentShell(), wizard);
			dialog.create();
			// TODO do we need a hook for providing custom help? Or would this just be shown
			// in the update browser?
			return dialog.open();
		}
		final UpdateWizard wizard = new UpdateWizard(ctx, this, operation, operation.getSelectedUpdates(), job);
		wizard.setRemediationOperation(remediationOperation);
...
//		PlatformUI.getWorkbench().getHelpSystem().setHelp(dialog.getShell(), IProvHelpContextIds.UPDATE_WIZARD);
	 ...
	 * @return the wizard return code
	 */
	public int openUninstallWizard(final Collection<IInstallableUnit> initialSelections, final UninstallOperation operation,
			final LoadMetadataRepositoryJob job) {
		final UninstallWizard wizard = new UninstallWizard(ctx, this, operation, initialSelections, job);
		final WizardDialog dialog = new ProvisioningWizardDialog(ctx, ProvUI.getDefaultParentShell(), wizard);
		dialog.create();
//		PlatformUI.getWorkbench().getHelpSystem().setHelp(dialog.getShell(), IProvHelpContextIds.UNINSTALL_WIZARD);
	 ...
	 */
	public void manipulateRepositories(final Shell shell) {
...
//			dialog.open();
//		} else {
			final TitleAreaDialog dialog = new TitleAreaDialog(shell) {
				RepositoryManipulationPage page;

				@Override
				protected Control createDialogArea(final Composite parent) {
					page = new RepositoryManipulationPage(ctx);
...
//		}
	 ...
	 */
	public void signalRepositoryOperationComplete(final RepositoryEvent event, final boolean update) {
		runner.eventBatchCount--;
		if (Tracing.DEBUG_EVENTS_CLIENT)
	...
	public IMetadataRepository loadMetadataRepository(final URI location, final boolean notify, final IProgressMonitor monitor)
			throws ProvisionException {
		IMetadataRepository repo = null;
		try {
			signalRepositoryOperationStart();
			final IMetadataRepositoryManager manager = ProvUI.getMetadataRepositoryManager(getSession());
	 ...
	 * @throws ProvisionException if the repository could not be loaded
	 */
	public IArtifactRepository loadArtifactRepository(final URI location, final boolean update, final IProgressMonitor monitor)
			throws ProvisionException {
		IArtifactRepository repo;
		signalRepositoryOperationStart();
		try {
			final IArtifactRepositoryManager manager = ProvUI.getArtifactRepositoryManager(getSession());
			repo = manager.loadRepository(location, monitor);
