...
public class FilteredTree extends Composite {
	 ...
	 * @since 3.116
	 */
	public FilteredTree(final Composite parent, final boolean useNewLook, final boolean useFastHashLookup) {
	...
	public FilteredTree(final Composite parent, final int treeStyle, final PatternFilter filter, final boolean useNewLook,
			final boolean useFastHashLookup) {
		super(parent, SWT.NONE);
		this.parent = parent;
	...
	protected void init(final int treeStyle, final PatternFilter filter) {
		...
		showFilterControls = InstanceScope.INSTANCE
			    .getNode(ProvUIAddon.PLUGIN_ID).getBoolean(SHOW_FILTERED_TEXTS, true);
	 ...
	 */
	private TreeItem getFirstMatchingItem(final TreeItem[] items) {
		for (final TreeItem item : items) {
			if (patternFilter.isLeafMatch(treeViewer, item.getData())
					&& patternFilter.isElementSelectable(item.getData())) {
				return item;
			}
			final TreeItem treeItem = getFirstMatchingItem(item.getItems());
			if (treeItem != null) {
				return treeItem;
			}
		}
	...
	protected WorkbenchJob doCreateRefreshJob() {
		return new WorkbenchJob("Refresh Filter") {//$NON-NLS-1$
			 ...
			 */
			private boolean recursiveExpand(final TreeItem[] items, final IProgressMonitor monitor, final long cancelTime,
					final int[] numItemsLeft) {
	...
	protected void createFilterText(final Composite parent) {
		...
		filterText.getAccessible().addAccessibleListener(new AccessibleAdapter() {
			 ...
			 * @return int
			 */
			private int itemCount(final TreeItem treeItem) {
				int count = 1;
				final TreeItem[] children = treeItem.getItems();
				for (final TreeItem element : children) {
		...
		filterText.addKeyListener(new KeyAdapter() {
			...
			public void keyPressed(final KeyEvent e) {
				...
				final boolean hasItems = getViewer().getTree().getItemCount() > 0;
	 ...
	 * @since 3.105
	 */
	protected void updateTreeSelection(final boolean setFocus) {
		...
		if (tree.getItemCount() == 0) {
			...
			// to match
			final boolean hasFocus = setFocus ? tree.setFocus() : true;
	 ...
	 * @since 3.105
	 */
	public void setQuickSelectionMode(final boolean enabled) {
