...
public abstract class ResolutionStatusPage extends ProvisioningWizardPage {
	 ...
	 * @param pageName
	 */
	protected ResolutionStatusPage(final IEclipseContext ctx, final String pageName, final ProvisioningUI ui,
			final ProvisioningOperationWizard wizard) {
	...
	public void updateStatus(final IUElementListRoot newRoot, final ProfileChangeOperation op) {
		final IStatus currentStatus = getProvisioningWizard().getCurrentStatus();
		...
		if (currentStatus != null && !currentStatus.isOK()) {
			...
			final int severity = currentStatus.getSeverity();
			if (severity == IStatus.ERROR) {
				messageType = IMessageProvider.ERROR;
				pageComplete = false;
				// Log errors for later support
				ProvUI.reportStatus(ctx, currentStatus, StatusManager.LOG);
	...
	protected String getIUDescription(final AvailableIUElement element) {
		final IInstallableUnit iu = element.getIU();
	...
	protected String getIUDescription(final Object element) {
		if (element instanceof AvailableIUElement) {
			return getIUDescription((AvailableIUElement) element);
		}
		final IInstallableUnit selectedIU = ElementUtils.elementToIU(element);
		if (selectedIU != null) {
	...
	String getMessageText(final IStatus currentStatus) {
		if (currentStatus == null || currentStatus.isOK())
	...
	void setDetailText(final ProfileChangeOperation resolvedOperation) {
		String detail = null;
		final Object selectedElement = getSelectedElement();
		final IInstallableUnit selectedIU = ElementUtils.elementToIU(selectedElement);
		final IUDetailsGroup detailsGroup = getDetailsGroup();
		...
		if (section != null) {
			try {
				if (section.get(NAME_COLUMN_WIDTH, null) != null)
					getNameColumn().setWidthInPixels(section.getInt(NAME_COLUMN_WIDTH, -1));
				if (section.get(VERSION_COLUMN_WIDTH, null) != null)
					getVersionColumn().setWidthInPixels(section.getInt(VERSION_COLUMN_WIDTH, -1));
				if (section.get(ID_COLUMN_WIDTH, null) != null)
					getIdColumn().setWidthInPixels(section.getInt(ID_COLUMN_WIDTH, -1));
			} catch (final NumberFormatException e) {
				// Ignore if there actually was a value that didn't parse.
			}
		}
	...
	public void saveBoundsRelatedSettings() {
		...
		final Preferences settings = InstanceScope.INSTANCE.getNode(ProvUIAddon.PLUGIN_ID);
		final Preferences section = settings.node(getDialogSettingsName());
		section.putInt(NAME_COLUMN_WIDTH, getNameColumnWidth());
		section.putInt(VERSION_COLUMN_WIDTH, getVersionColumnWidth());
		section.putInt(ID_COLUMN_WIDTH, getIdColumnWidth());
		final int[] weights = getSashForm().getWeights();
	...
	private boolean isLocked(final AvailableIUElement elementToBeUpdated) {
