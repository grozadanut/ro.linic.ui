...
public class ComboAutoCompleteField {
	...
	public ComboAutoCompleteField(final Combo c) {
		this.combo = c;
	...
	public void setProposalStrings(final String[] proposals) {
	...
	IContentProposalProvider getProposalProvider() {
		return (contents, position) -> {
			final String[] items = getStringItems();
			if (contents.length() == 0 || items.length == 0)
				return new IContentProposal[0];
			final StringMatcher matcher = new StringMatcher("*" + contents + "*", true, false); //$NON-NLS-1$ //$NON-NLS-2$
