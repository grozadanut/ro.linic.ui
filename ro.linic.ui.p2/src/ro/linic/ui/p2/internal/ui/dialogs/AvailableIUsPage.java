...
import ro.linic.ui.p2.internal.ui.viewers.IDeferredQueryTreeListener;
...
//	Link installLink;
	...
	public AvailableIUsPage(final IEclipseContext ctx, final ProvisioningUI ui, final ProvisioningOperationWizard wizard) {
	...
	public void createControl(final Composite parent) {
		...
		final Composite aboveSash = new Composite(sashForm, SWT.NONE);
		...
		availableIUGroup = new AvailableIUGroup(ctx, getProvisioningUI(), aboveSash, JFaceResources.getDialogFont(), queryContext, new IUColumnConfig[] {nameColumn, versionColumn}, filterConstant);
		...
		final Composite progressBarComposite = createProgressBar(composite);
	...
	private void createSelectionCount(final Composite parentComposite) {
		final Composite selectionCountComposite = new Composite(parentComposite, SWT.NONE);
		...
		final Label sep = new Label(selectionCountComposite, SWT.HORIZONTAL | SWT.SEPARATOR);
		final GridData separatorGridData = new GridData(SWT.FILL, SWT.CENTER, true, false);
	...
	private Composite createProgressBar(final Composite parent) {
		...
		final Label progressBarLabel = new Label(progressBarComposite, SWT.NONE);
		progressBarLabel.setText(ProvUIMessages.AvailableIUsPage_Fetching);
		new ProgressBar(progressBarComposite, SWT.INDETERMINATE);
		final IContentProvider contentProvider = availableIUGroup.getCheckboxTreeViewer().getContentProvider();
	...
	private void createSelectButtons(final Composite parent) {
		...
		final GridData data = new GridData(SWT.FILL, SWT.DEFAULT, false, false);
	...
	void setAllChecked(final boolean checked) {
		if (checked) {
	...
	private void createViewControlsArea(final Composite parent) {
...
//		installLink.setLayoutData(gd);
	...
	private void createRepoArea(final Composite parent) {
	...
	void repoComboSelectionChanged(final int repoChoice, final URI repoLocation) {
	...
	void updateSelection() {
		final int count = availableIUGroup.getCheckedLeafIUs().length;
		setPageComplete(count > 0);
		if (count == 0) {
			selectionCount.setText(""); //$NON-NLS-1$
		} else {
			final String message = count == 1 ? ProvUIMessages.AvailableIUsPage_SingleSelectionCount : ProvUIMessages.AvailableIUsPage_MultipleSelectionCount;
	...
	private void setDropTarget(final Control control) {
		if (getPolicy().getRepositoriesVisible()) {
			...
			target.addDropListener(new RepositoryManipulatorDropTarget(ctx, getProvisioningUI(), control));
		}
	...
	private void initializeWidgetState() {
		...
		if (resolveAllCheckbox != null) {
			...
			final Preferences section = settings.node(DIALOG_SETTINGS_SECTION);
			String value = null;

			if (section != null) {
				value = section.get(RESOLVE_ALL, null);
				resolveAllCheckbox.setSelection(section.getBoolean(RESOLVE_ALL, false));
			}
	...
	private void makeQueryContext() {
		...
		if (section != null) {
			// View by...
			try {
				if (section.get(AVAILABLE_VIEW_TYPE, null) != null) {
					queryContext.setViewType(section.getInt(AVAILABLE_VIEW_TYPE, IUViewQueryContext.AVAILABLE_VIEW_BY_CATEGORY));
				}
			...
			if (section.get(SHOW_LATEST_VERSIONS_ONLY, null) != null) {
				queryContext.setShowLatestVersionsOnly(section.getBoolean(SHOW_LATEST_VERSIONS_ONLY, true));
			}

			// Hide installed content
			if (section.get(HIDE_INSTALLED_IUS, null) != null) {
				queryContext.setHideAlreadyInstalled(section.getBoolean(HIDE_INSTALLED_IUS, false));
			}

			if (section.get(FILTER_ON_ENV, null) != null) {
				queryContext.setFilterOnEnv(section.getBoolean(FILTER_ON_ENV, false));
			}
		}
	}
	...
	private void getColumnWidthsFromSettings() {
		...
		if (section != null) {
			try {
				if (section.get(NAME_COLUMN_WIDTH, null) != null) {
					nameColumn.setWidthInPixels(section.getInt(NAME_COLUMN_WIDTH, -1));
				}
				if (section.get(VERSION_COLUMN_WIDTH, null) != null) {
					versionColumn.setWidthInPixels(section.getInt(VERSION_COLUMN_WIDTH, -1));
				}
			} catch (final NumberFormatException e) {
				// Ignore if there actually was a value that didn't parse.
			}
		...
		if (section != null) {
			try {
				...
				if (section.get(LIST_WEIGHT, null) != null) {
					weights[0] = section.getInt(LIST_WEIGHT, 80);
					if (section.get(DETAILS_WEIGHT, null) != null) {
						weights[1] = section.getInt(DETAILS_WEIGHT, 20);
						return weights;
					}
				}
			} catch (final NumberFormatException e) {
	...
	public void saveBoundsRelatedSettings() {
		...
		final Preferences settings = InstanceScope.INSTANCE.getNode(ProvUIAddon.PLUGIN_ID);
		final Preferences section = settings.node(DIALOG_SETTINGS_SECTION);
		section.putInt(AVAILABLE_VIEW_TYPE, queryContext.getViewType());
		section.putBoolean(SHOW_LATEST_VERSIONS_ONLY, showLatestVersionsCheckbox.getSelection());
		section.putBoolean(HIDE_INSTALLED_IUS, hideInstalledCheckbox.getSelection());
		if (resolveAllCheckbox != null) {
			section.putBoolean(RESOLVE_ALL, resolveAllCheckbox.getSelection());
		}
		section.putBoolean(FILTER_ON_ENV, filterOnEnvCheckBox.getSelection());

		TreeColumn col = availableIUGroup.getCheckboxTreeViewer().getTree().getColumn(0);
		section.putInt(NAME_COLUMN_WIDTH, col.getWidth());
		col = availableIUGroup.getCheckboxTreeViewer().getTree().getColumn(1);
		section.putInt(VERSION_COLUMN_WIDTH, col.getWidth());

		final int[] weights = sashForm.getWeights();
		section.putInt(LIST_WEIGHT, weights[0]);
		section.putInt(DETAILS_WEIGHT, weights[1]);
	...
	void updateDetails() {
		...
		final Object[] elements = availableIUGroup.getStructuredViewer().getStructuredSelection().toArray();
		if (elements.length == 1 && elements[0] instanceof EmptyElementExplanation) {
			final String description = ((EmptyElementExplanation) elements[0]).getDescription();
			if (description != null) {
		...
		// Now look for IU's
		final java.util.List<IInstallableUnit> selected = getSelectedIUs();
		if (selected.size() == 1) {
			final StringBuilder result = new StringBuilder();
			final String filterString = availableIUGroup.getFilterString();
			final String description = selected.get(0).getProperty(IInstallableUnit.PROP_DESCRIPTION, null);
			if (description != null) {
				result.append(description);
			} else {
				final String name = selected.get(0).getProperty(IInstallableUnit.PROP_NAME, null);
				if (name != null) {
					result.append(name);
				} else {
	...
	void addViewerProvisioningListeners() {
		...
		profileListener = new StructuredViewerProvisioningListener(ctx, getClass().getName(), availableIUGroup.getStructuredViewer(), ProvUIProvisioningListener.PROV_EVENT_PROFILE, getProvisioningUI().getOperationRunner()) {
			@Override
			protected void profileAdded(final String id) {
				// do nothing
			}

			@Override
			protected void profileRemoved(final String id) {
				// do nothing
			}

			@Override
			protected void profileChanged(final String id) {
	...
	protected String getClipboardText(final Control control) {
		...
		return CopyUtils.getIndentedClipboardText(getSelectedIUElements(), new IUDetailsLabelProvider(ctx));
	}
