...
import ro.linic.ui.p2.org.eclipse.ui.DeferredTreeContentManager;
import ro.linic.ui.p2.org.eclipse.ui.IDeferredWorkbenchAdapter;
import ro.linic.ui.p2.org.eclipse.ui.PendingUpdateAdapter;
...
public class DeferredQueryTreeContentManager extends DeferredTreeContentManager {
	...
	class ElementPendingUpdateAdapter extends PendingUpdateAdapter {
		...
		ElementPendingUpdateAdapter(final Object element) {
			super();
		...
		@Override
		public void setRemoved(final boolean removedValue) {
	...
	public DeferredQueryTreeContentManager(final AbstractTreeViewer viewer) {
		super(viewer);
	...
	protected void addChildren(final Object parent, final Object[] children, final IProgressMonitor monitor) {
		...
		final Job updateJob = org.eclipse.e4.ui.progress.UIJob.create("Adding children", updateMonitor -> { //$NON-NLS-1$
	...
	protected void runClearPlaceholderJob(final PendingUpdateAdapter placeholder) {
		if (placeholder instanceof ElementPendingUpdateAdapter) {
			final ElementPendingUpdateAdapter pendingUpdate = (ElementPendingUpdateAdapter) placeholder;
			if (pendingUpdate.isRemoved()) {
				return;
			}
			notifyListener(false, (ElementPendingUpdateAdapter) placeholder);
			final Job clearJob = org.eclipse.e4.ui.progress.UIJob.create("Clearing", monitor -> { //$NON-NLS-1$
				if (!pendingUpdate.isRemoved()) {
	...
	public void addListener(final IDeferredQueryTreeListener listener) {
		if (listener != null) {
			this.listeners.add(listener);
		}
	}

	private void notifyListener(final boolean starting, final ElementPendingUpdateAdapter placeholder) {
		if (listeners == null || listeners.isEmpty())
			return;
		if (starting) {
			for (final IDeferredQueryTreeListener deferredQueryTreeListener : listeners) {
