...
public abstract class ProvUIProvisioningListener implements SynchronousProvisioningListener {
	...
	public ProvUIProvisioningListener(final String name, final int eventTypes, final ProvisioningOperationRunner runner) {
		this.name = name;
	...
	public void notify(final EventObject o) {
		if (o instanceof RepositoryOperationBeginningEvent) {
			...
			final RepositoryOperationEndingEvent event = (RepositoryOperationEndingEvent) o;
			...
			if (runner.eventBatchCount <= 0) {
				...
				final RepositoryEvent innerEvent = event.getEvent();
				if (innerEvent != null) {
			...
			final IProfileEvent event = (IProfileEvent) o;
	 ...
	 * @param event
	 */
	protected void repositoryEnablement(final RepositoryEvent event) {
