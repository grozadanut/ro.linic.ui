...
import static ro.linic.ui.legacy.session.UIUtils.extractLocalTime;
import static ro.linic.ui.legacy.session.UIUtils.insertTime;
 ...
 * </p>
 */
public class LocalTimeCellEditor extends AbstractCellEditor
{
	/**
	 ...
	 * @since 1.6
	 */
	private LocalTime currentEditorValue;
     ...
     */
    public LocalTimeCellEditor(final boolean moveSelectionOnEnter)
    {
	...
	{
		this.currentEditorValue = extractLocalTime(this.dateTime);
	...
	public void setEditorValue(final Object value)
	{
		...
		{
			this.currentEditorValue = (LocalTime) value;
			insertTime(this.dateTime, this.currentEditorValue);
		}
	}
	...
	{
		LocalTime editorValue = null;
		if (canonicalValue instanceof LocalTime)
			editorValue = (LocalTime) canonicalValue;
	...
	{
		final DateTime dateControl = new DateTime(parent, SWT.TIME | SWT.SHORT);
		...
		dateControl.addKeyListener(new KeyAdapter()
		{
			@Override public void keyPressed(final KeyEvent event)
			{
				if (event.keyCode == SWT.CR || event.keyCode == SWT.KEYPAD_CR)
				{
					...
					if (LocalTimeCellEditor.this.moveSelectionOnEnter && LocalTimeCellEditor.this.editMode == EditModeEnum.INLINE)
					{
					...
					if (LocalTimeCellEditor.this.editMode == EditModeEnum.DIALOG)
					{
						parent.forceFocus();
					}
