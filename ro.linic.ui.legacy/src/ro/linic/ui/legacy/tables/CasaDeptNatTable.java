...
public class CasaDeptNatTable
{
	private static final Column nameColumn = new Column(0, CasaDepartment.NAME_FIELD, "Nume", 100);
	private static final Column cotaTvaColumn = new Column(1, CasaDepartment.COTA_TVA_FIELD, "Grupa TVA", 100);
	private static final Column tvaPercentageColumn = new Column(2, CasaDepartment.TVA_PERCENTAGE_FIELD, "%TVA", 100);
	private static final Column idxColumn = new Column(3, CasaDepartment.ID_FIELD, "idx", 90);
	
	private static ImmutableList<Column> ALL_COLUMNS = ImmutableList.<Column>builder()
			.add(nameColumn)
			.add(cotaTvaColumn)
			.add(tvaPercentageColumn)
			.add(idxColumn)
			.build();
	
	private EventList<CasaDepartment> sourceData;
	private NatTable table;
	
	private SelectionLayer selectionLayer;
	private ViewportLayer viewportLayer;
	
	public CasaDeptNatTable()
	{
	}

	public void postConstruct(final Composite parent)
	{
		final IColumnPropertyAccessor<CasaDepartment> columnAccessor =
        ...
        final TransformedList<CasaDepartment, CasaDepartment> rowObjectsGlazedList = GlazedLists.threadSafeList(sourceData);
        final SortedList<CasaDepartment> sortedData = new SortedList<>(rowObjectsGlazedList, null);

		// create the body layer stack
		final IRowDataProvider<CasaDepartment> bodyDataProvider = new ListDataProvider<>(sortedData, columnAccessor);
		...
		final GlazedListsEventLayer<CasaDepartment> glazedListsEventLayer = new GlazedListsEventLayer<>(bodyDataLayer, sortedData);
		...
		final SortHeaderLayer<CasaDepartment> sortHeaderLayer = new SortHeaderLayer<CasaDepartment>(columnHeaderLayer,
				new GlazedListsSortModel<CasaDepartment>(sortedData, columnAccessor, configRegistry, columnHeaderDataLayer));
		...
		selectionLayer.setSelectionModel(
				new RowSelectionModel<>(selectionLayer, bodyDataProvider, new IRowIdAccessor<CasaDepartment>()
				{
					@Override public Serializable getRowId(final CasaDepartment rowObject)
					{
	...
	public CasaDeptNatTable loadData(final ImmutableList<CasaDepartment> data)
	{
		try
		{
	...
	public CasaDeptNatTable add(final CasaDepartment newDept)
	{
		try
		{
			this.sourceData.getReadWriteLock().writeLock().lock();
			sourceData.add(newDept);
			table.refresh();
		}
		finally
		{
	...
	public CasaDeptNatTable remove(final CasaDepartment uiDept)
	{
		try
		{
			this.sourceData.getReadWriteLock().writeLock().lock();
			sourceData.remove(uiDept);
			table.refresh();
		}
		finally
		{
	...
	public EventList<CasaDepartment> getSourceData()
	{
		return sourceData;
	}
	...
	public List<CasaDepartment> selection()
	{
		return ((RowSelectionModel<CasaDepartment>) selectionLayer.getSelectionModel()).getSelectedRowObjects();
	}
	...
	private class CustomStyleConfiguration extends AbstractRegistryConfiguration
	{
		...
		{
			// Display converters
			configRegistry.registerConfigAttribute(CellConfigAttributes.DISPLAY_CONVERTER, new PercentageBigDecimalDisplayConverter(true), DisplayMode.NORMAL, 
					ColumnLabelAccumulator.COLUMN_LABEL_PREFIX + ALL_COLUMNS.indexOf(tvaPercentageColumn));
			...
			configRegistry.registerConfigAttribute(EditConfigAttributes.CELL_EDITABLE_RULE,
					IEditableRule.ALWAYS_EDITABLE, DisplayMode.NORMAL, ColumnLabelAccumulator.COLUMN_LABEL_PREFIX + ALL_COLUMNS.indexOf(cotaTvaColumn));
			configRegistry.registerConfigAttribute(EditConfigAttributes.CELL_EDITABLE_RULE,
					IEditableRule.ALWAYS_EDITABLE, DisplayMode.NORMAL, ColumnLabelAccumulator.COLUMN_LABEL_PREFIX + ALL_COLUMNS.indexOf(tvaPercentageColumn));
