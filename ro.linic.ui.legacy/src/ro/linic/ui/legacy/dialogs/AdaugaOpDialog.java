...
public class AdaugaOpDialog extends Dialog
{
	private Point initialLocation;
	private UISynchronize sync;
	private Bundle bundle;
	private Logger log;
	private BiConsumer<Operatiune, Optional<Product>> adaugaOpConsumer;
	...
	public AdaugaOpDialog(final Shell parentShell, final Point initialLocation, final UISynchronize sync, final Bundle bundle, final Logger log,
			final BiConsumer<Operatiune, Optional<Product>> adaugaOpConsumer, final Supplier<AccountingDocument> accDocSupplier)
	{
		super(parentShell);
		...
		this.tvaPercent = Optional.ofNullable(produsSelectat)
				.map(Product::getDepartment)
				.map(CasaDepartment::getTvaPercentage)
				.orElseGet(() -> new BigDecimal(BusinessDelegate.persistedProp(PersistedProp.TVA_PERCENT_KEY)
	...
	protected Control createDialogArea(final Composite parent)
	{
		...
		GridDataFactory.fillDefaults().grab(true, true).span(2, 1).minSize(850, 200).applyTo(allProductsTable.getTable());
	...
	private void createRightContainer(final Composite parent)
	{
		...
		rightContainer.setTabList(new Control[]{puaFaraTVA, valAchFaraTVA, valAchTVA, pvCuTVA, valVanzFaraTVA, valVanzTVA});
	}
	...
	private void addListeners()
	{
		...
		autoCode.addSelectionListener(new SelectionAdapter()
		{
			...
			{
				barcodeName.setText(String.valueOf(BusinessDelegate.firstFreeNumber(Product.class, Product.BARCODE_FIELD)));
			}
		});
		...
		puaFaraTVA.addModifyListener(e -> updateAmounts());
		...
		applyPercentagePopup(valAchTVA, () -> parse(valAchFaraTVA.getText()), result -> valAchTVA.setText(result.toString()));
		applyPercentagePopup(valVanzTVA, () -> parse(valVanzFaraTVA.getText()), result -> valVanzTVA.setText(result.toString()));
	...
	{
		final BigDecimal valAchFaraTva = parse(puaFaraTVA.getText())
				.multiply(parse(cantitate.getText()))
				.setScale(2, RoundingMode.HALF_EVEN);
		final BigDecimal valVanzFaraTva = parse(pvCuTVA.getText())
				.multiply(parse(cantitate.getText()))
				.divide(tvaExtractDivisor, 2, RoundingMode.HALF_EVEN);
		final BigDecimal valVanzTva = parse(pvCuTVA.getText())
				.multiply(parse(cantitate.getText()))
				.subtract(valVanzFaraTva)
				.setScale(2, RoundingMode.HALF_EVEN);
	...
	private void adaugaOp()
	{
		if (isValid())
		{
			...
			adaugaOpConsumer.accept(op, Optional.of(produsSelectat));
	...
	private void adaugaDiscount()
	{
		if (greaterThan(parse(discountValue.getText()), BigDecimal.ZERO) && accDocSupplier.get() != null)
		{
			...
			adaugaOpConsumer.accept(op, Optional.empty());
			close();
		}
	}
	...
	private String calculateDiscount()
	{
		...
		return parse(discountPercent.getText())
