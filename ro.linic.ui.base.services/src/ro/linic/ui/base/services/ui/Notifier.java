...
import ro.linic.ui.base.services.ui.NotifierColorsFactory.NotifierTheme;
...
public class Notifier {
	...
	public static void notify(final String title, final String text) {
		notify(null, title, text, NotifierTheme.FLEXBIZ_THEME, 8000, null);
	}
	 ...
	 * @param title the title of the popup window
	 * @param text  the text of the notification
	 * @param displayTime time in ms after which to close the window, -1 to disable auto closing
	 *
	 */
	public static void notify(final String title, final String text, final int displayTime) {
		notify(null, title, text, NotifierTheme.FLEXBIZ_THEME, displayTime, null);
	}
	 ...
	 *
	 */
	public static void notify(final String title, final String text, final int displayTime, final Consumer<Object> callback) {
		notify(null, title, text, NotifierTheme.FLEXBIZ_THEME, displayTime, callback);
	}
	 ...
	 *
	 */
	public static void notify(final Image image, final String title, final String text, final Consumer<Object> callback) {
		notify(image, title, text, NotifierTheme.FLEXBIZ_THEME, 8000, callback);
	 ...
	 * @see NotifierTheme
	 */
	public static void notify(final String title, final String text, final NotifierTheme theme, final int displayTime, final Consumer<Object> callback) {
		notify(null, title, text, theme, displayTime, callback);
	}
	 ...
	 * @see NotifierTheme
	 */
	public static void notify(final Image image, final String title, final String text, final NotifierTheme theme, final int displayTime,
			final Consumer<Object> callback) {
		final Shell shell = createNotificationWindow(image, title, text,
				NotifierColorsFactory.getColorsForTheme(theme), callback);
		makeShellAppears(shell, displayTime);
	}
	...
	protected static Shell createNotificationWindow(final Image image, final String title, final String text,
			final NotifierColors colors, final Consumer<Object> callback) {
		...
		createShellAction(shell, callback);
	...
	private static void createBackground(final Shell shell, final NotifierColors colors) {
		shell.addListener(SWT.Resize, event -> {
			...
			final Optional<Image> closeImage = UIUtils.find(FrameworkUtil.getBundle(Notifier.class), "icons/delete_16x16.png") //$NON-NLS-1$
					.map(GUIHelper::getImageByURL); 
			closeImage.ifPresent(img -> gc.drawImage(img, rect.width - 21, 2));

			gc.dispose();
//			closeImage.ifPresent(Image::dispose);
	 ...
	 */
	protected static void makeShellAppears(final Shell shell, final int displayTime) {
	 ...
	 */
	private static void createShellAction(final Shell shell, final Consumer<Object> callback) {
		shell.addListener(SWT.MouseUp, event -> {
			final Rectangle rect = shell.getClientArea();
			final int xUpperLeftCorner = rect.width - 21;
			final int yUpperLeftCorner = 2;

			if (event.x >= xUpperLeftCorner && event.x <= xUpperLeftCorner + 16 && event.y >= yUpperLeftCorner
